...
// eminates radially outward in random directions.
//
ImpulseRadialForceRing::ImpulseRadialForceRing(float magnitude) {
	this->magnitude = magnitude;
	applyOnce = true;
}

void ImpulseRadialForceRing::updateForce(Particle * particle) {

	// we basically create a random direction for each particle
	// the force is only added once after it is triggered.
	//
	ofVec3f dir = ofVec3f(ofRandom(-1, 1), ofRandom(0, 0), ofRandom(-1, 1)); // ofRandom(0, 0) makes sure that a ring effect is created
	particle->forces += dir.getNormalized() * magnitude;
}